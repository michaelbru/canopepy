...
from pdu import CanopenRequest
from pdu import CanopenResponse
from pdu import CanopenExceptions as merror
...
class SDOUploadRequest(ModbusRequest):
    '''
    ...
    ccs = 2
    _rtu_frame_size = 8
    canId = Defaults.canIds['Transmit SDO'][0]-1

    def __init__(self,nodeId,index,subindex,**kwargs):
        ''' Initializes a new instance
        :param nodeId: The variable address to read
        :param index:  OD entry
        :param subindex: OD subentry - together with index defines 
        what operation the node had been required
        :param value: None
        '''
        CanopenRequest.__init__(self, **kwargs)
        self.nodeId = nodeId
        self.cobId = self.canId + self.nodeId
        self.index = index
        self.subindex = subindex


    def encode(self,data):
        ''' Encodes read request
...
class SDOUploadResponse(CanopenResponse):
    '''
