...
from exceptions import CanopenIOException
from constants  import Defaults
from interfaces import ICanopenMessage
...
#---------------------------------------------------------------------------#
class CanopenTransactionManager(object):
    ...
    def __init__(self, client, **kwargs):
        ''' Initializes an instance of the CanopenTransactionManager
    ...
    def execute(self, request):
        ...
        request.transaction_id = self.getId()
...
class DictTransactionManager(CanopenTransactionManager):
...
class FifoTransactionManager(CanopenTransactionManager):
    ''' Impelements a transaction for a manager where the
...
# Canopen Message
#---------------------------------------------------------------------------#
class SdoInitiateUploadRequestMessage(ICanopenMessage):
    ''' Canopen Message controller
    ...
    def __init__(self):
        ...
        '''
        self.__buffer = ''
        self.__header = {'ccs':0, 'x':0, 'index':0, 'subindex':0}
        self.__hsize  = 0x07
    ...
    def populateResult(self, result):
        '''
        Populates the Canopen result with the transport specific header
    ...
    def processIncomingPacket(self, data, callback):
        ...
        while self.isFrameReady():
            if self.checkMessage():
                ...
                if result is None:
                    raise CanopenIOException("Unable to decode request")
    ...
    def buildMessage(self, message):
        ''' Creates a ready to send Canopen message
...
# Canopen RTU Message
#---------------------------------------------------------------------------#
class CanopenRtuFramer(ICanopenMessage):
    '''
    Canopen RTU Frame controller::
    ...
    def populateResult(self, result):
        ''' Populates the Canopen result header
    ...
    def processIncomingPacket(self, data, callback):
        ...
        while self.isFrameReady():
            if self.checkFrame():
                result = self.decoder.decode(self.getFrame())
                if result is None:
                    raise CanopenIOException("Unable to decode response")
                self.populateResult(result)
                self.advanceFrame()
                callback(result)  # defer or push to a thread?
    ...
    def buildPacket(self, message):
        ''' Creates a ready to send Canopen packet
...
#---------------------------------------------------------------------------#
# Canopen ASCII Message
#---------------------------------------------------------------------------#
class CanopenAsciiFramer(ICanopenMessage):
    '''
    Canopen ASCII Frame Controller::
    ...
    def buildPacket(self, message):
        ''' Creates a ready to send Canopen packet
        Built off of a  Canopen request/response
...
#---------------------------------------------------------------------------#
# Canopen Binary Message
#---------------------------------------------------------------------------#
class CanopenBinaryFramer(ICanopenMessage):
    '''
    Canopen Binary Frame Controller::
...
__all__ = [
    ...
    "CanopenSocketFramer", "CanopenRtuFramer",
    "CanopenAsciiFramer", "CanopenBinaryFramer",
]
